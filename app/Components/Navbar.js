...
const Navbar = () => {
  ...
  return (
    <nav
      className={`bg-gray-900 px-8 text-white transition-all duration-500 ease-in-out ${
        dropdown ? "py-6" : "py-3"
      } `}
    >
      <div className="flex justify-between items-center">
      <Link href={'/'}>
      <div className="logo font-bold text-2xl">GetMeAChai!</div></Link> 
        <div className="butons flex justify-between gap-4 relative">
          {session && (
            <div className="dropdown">
              <button
                onClick={() => setDropdown(!dropdown)} onBlur={()=>{setDropdown(!dropdown)}}
              ...
              <div
                id="dropdown"
                className={`transition-all duration-300 ease-in-out overflow-hidden absolute mt-2 w-44 bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 ${
